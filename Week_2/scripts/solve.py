...
# Move stores (x, y, side) i.e number of missionaries, cannibals to be moved from left to right or vice versa for paritcular state
...
class Solution():
    ...
    def write_image(self, file_name="state_space.png"):
        ...
        print(f"File {file_name}.png succesfully written")

    def solve(self, solve_method="bfs"):
        self.visited = dict()
    ...
    def draw_legend(self):
        """
        Utility method to draw legend on flag if legend flag is ON
        """
        graphlegend = pydot.Cluster(graph_name="legend", label="Legend", fontsize="20", fontcolor="blue", 
                                    color="gold", style="filled", fillcolor="#f4f4f4")

        node1 = pydot.Node("1", style="filled", fillcolor="blue", label="Start Node",
                           fontcolor="white", width="2", fixedsize="true")
        graphlegend.add_node(node1)

        node2 = pydot.Node("2", style="filled", fillcolor="red", label="Killed Node",
                           fontcolor="black", width="2", fixedsize="true")
        graphlegend.add_node(node2)

        node3 = pydot.Node("3", style="filled", fillcolor="yellow", label="Solution Nodes",
                            width="2", fixedsize="true")
        graphlegend.add_node(node3)

        node4 = pydot.Node("4", style="filled", fillcolor="gray", label="Can't be Expanded",
                            width="2", fixedsize="true")
        graphlegend.add_node(node4)

        node5 = pydot.Node("5", style="filled", fillcolor="green", label="Goal Node",
                            width="2", fixedsize="true")
        graphlegend.add_node(node5)

        node7 = pydot.Node("7", style="filled", fillcolor="gold", label="Node with child",
                            width="2", fixedsize="true")
        ...
        description = "Each node (m, c, s) represents a \nstate where 'm' is the number of\n missionaries, 'n' the cannibals and \n's' the side of the boat.\n"\
            "'s' is 1 if\n the boat is on the left\n side and 0 if the boat is on the right side. \n\nOur objective is to reach the goal state\n (0, 0, 0) \nfrom start state (3, 3, 1) by some \noperators = [(1, 0), (0, 1), (1, 1), (0, 2), (2, 0)]\n"\
                "each tuple (x, y) inside operators \nrepresents the number of missionaries\n and cannibals to be moved from left to right\nif c == 1 and vice versa.\n\n"
        
        node6 = pydot.Node("6", style="filled", fillcolor="gold", label=description,
                            shape="plaintext", fontsize="20", fontcolor="red")
    ...
    def draw(self, *, number_missionaries_left, number_cannibals_left, 
             number_missionaries_right, number_cannibals_right):
        """
        Draw state on console using emojis
        """
        left_m = emoji.emojize(f":old man: " * number_missionaries_left)       
        left_c = emoji.emojize(f"ogre: " * number_cannibals_left) 
        right_m = emoji.emojize(f":old man: " * number_missionaries_right)
        right_c = emoji.emojize(f"ogre: " * number_cannibals_right)

        print("{}{}{}{}{}".format(left_m, left_c + " " * (14 - len(left_m) - len(left_c)), "_" * 40, 
                                  " " * (12 - len(right_m) - len(right_c)) + right_m, right_c))
        print("")
    ...
    def show_solution(self):
        ...
        for i, ((number_missionaries, number_cannibals, side), node) in enumerate(zip(steps[1:], nodes[1:])):
            ...
            print(f"Step {i + 1}: Move {number_missionaries} missionaries and {number_cannibals} cannibals" +
                  f"from {self.boat_side[side]} to {self.boat_side[int(not side)]}")
